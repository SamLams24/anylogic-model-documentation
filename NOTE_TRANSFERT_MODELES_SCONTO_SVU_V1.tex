...
\code{...\_DATACO\_\allowbreak MULTIPRODUCT\_\allowbreak CONCURRENT\_\allowbreak FIX.alp} (migration, 2026-09-15) & 2\,371\,853 & Présentes & \textbf{Une seule différence identifiée avec \code{SCONTO\_SVU\_\allowbreak DATACO\_FINAL.alp} : le champ interne \code{<Name>}, voir ci-dessous} \\
...
Après normalisation de ce seul champ, les deux fichiers sont identiques.
\textbf{Conclusion démontrée, non supposée} : \code{SCONTO\_SVU\_\allowbreak DATACO\_FINAL.alp}
est une copie renommée de cette variante, sans aucune autre modification.

Les deux fichiers conservent néanmoins des empreintes SHA-256 distinctes,
précisément parce que le champ \code{<Name>} diffère : un hash
cryptographique change dès qu'un seul octet du fichier change, y compris
pour un champ de métadonnées sans effet fonctionnel. L'identité
fonctionnelle démontrée ci-dessus repose sur la comparaison de contenu
après normalisation de ce champ, pas sur une égalité des empreintes
brutes.
...
\code{anylogic-model-\allowbreak documentation} \newline \code{documentation-\allowbreak latex-rewrite} & \code{a10652d} & HEAD après ajout de la présente note ; documents scientifiques DataCo/Prophet/Recalibrator à jour depuis le commit \code{3698c14} \\
\bottomrule
\end{tabularx}
\end{center}
...
\code{SCONTO\_SVU\_\allowbreak DATACO\_FINAL.alp} & hors dépôt structuré (livraison locale) & n/d & Modèle DataCo exécuté pour RUN~1/RUN~2 & Non applicable & Identique après normalisation du champ \code{<Name>} à la variante migration 2026-09-15 \\
