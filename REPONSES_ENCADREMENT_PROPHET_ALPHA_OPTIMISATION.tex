...
\noindent Le module DataCo de SCONTO-SVU articule trois volets : la construction de la prévision Prophet, la recalibration adaptative par $\alpha_j$, et la formulation des expérimentations d'optimisation E4/E5. Les résultats numériques cités proviennent du run de référence RUN 1 (\code{simToRealSeconds = 3600}).
...
Le dernier trimestre illustre le rôle du Recalibrator. En octobre, Prophet maintient une anticipation d'environ 10\,910 unités alors que la demande réelle s'établit à 4\,471 unités. La baseline Prophet est calculée avant le run à partir des données d'apprentissage et reste figée pendant la campagne : elle ne se réentraîne donc pas en cours d'année à partir de la demande 2017 observée. Lorsque le régime de demande se contracte fortement au dernier trimestre, la baseline reste proche de la trajectoire apprise, tandis que le Recalibrator incorpore progressivement les observations nouvelles. Le Recalibrator ramène ainsi la prévision à environ 4\,655 unités, soit une erreur résiduelle d'environ 183,71 unités contre environ 6\,438,6 unités pour Prophet seul. Le mécanisme ne remplace pas Prophet ; il transfère progressivement le poids de la prévision vers la demande observée à mesure que celle-ci devient disponible, ce qui est précisément l'objet de la section suivante.
...
Pour $j=1,\dots,N$, le rythme moyen observé jusqu'au jour $j$ s'écrit $r_j = R_j/j$, où $R_j$ est la demande cumulée observée au jour $j$. La projection de fin de mois prolonge ce rythme sur les $N-j$ jours restants :
\[
F_{proj}(j) = R_j + r_j\,(N-j) = R_j\times\frac{N}{j}, \qquad j=1,\dots,N
\]
Cette expression n'est pas définie en $j=0$, faute d'observation. Exemple : pour $R_j=4\,500$ au jour $j=10$ d'un mois de $N=30$ jours, $r_{10}=450$ unités par jour et $F_{proj}(10)=4\,500\times30/10=13\,500$ unités.
...
\]
Les propriétés développées ci-dessous justifient la cohérence de la famille $\alpha_j=(j/N)^{\gamma}$ et, pour $0<\gamma<1$, son comportement concave ; elles ne démontrent pas à elles seules que $\gamma=1/2$ est le meilleur choix parmi toutes les valeurs possibles. Le choix $\gamma=1/2$ constitue donc un choix de conception heuristique et transparent : il produit une montée de confiance plus rapide que la loi linéaire tout en conservant une progression concave. Sa valeur optimale éventuelle doit être évaluée par analyse de sensibilité.

\subsection{Propriétés mathématiques}

\paragraph{A. Bornage.} Pour $0\le j\le N$, $0\le j/N\le1$, donc $0\le\alpha_j\le1$.

\paragraph{B. Extrémités.} $\alpha_N=1$, et $\alpha_0=0$ par continuité / extension naturelle de $\alpha_j$ à $j=0$ : en début de mois, Prophet porte la prévision ; en fin de mois, la projection issue du réel la porte entièrement.

\paragraph{C. Monotonie, pour $j>0$.}
\[
\frac{d\alpha}{dj} = \frac{\gamma}{N}\left(\frac{j}{N}\right)^{\gamma-1} > 0
\]

\paragraph{D. Concavité pour $\gamma=1/2$ et $j>0$.}
\[
\frac{d^2\alpha}{dj^2} = \frac{\gamma(\gamma-1)}{N^2}\left(\frac{j}{N}\right)^{\gamma-2} < 0
\]
Ces deux dérivées ne sont pas définies en $j=0$, où $(j/N)^{\gamma-1}$ diverge pour $0<\gamma<1$ ; elles sont annoncées pour $j>0$, cohérent avec le domaine de définition de $F_{proj}$.
...
Pour chaque famille simulée $p$, le vecteur de décision est $x_p=(SS_p,\,s_p,\,Q_p,\,f_p)$, où $SS_p$ est le stock de sécurité, $s_p$ le point de commande, $Q_p$ la quantité de réapprovisionnement et $f_p$ la fréquence de révision du plan. $SS_p$ et $s_p$ peuvent être initialisés à partir des formules de Hadley et Whitin \cite{hadleywhitin1963}, qui relient stock de sécurité et point de commande aux statistiques de demande et de délai ; E5 les traite ensuite comme des variables de décision à optimiser plutôt que comme des valeurs fixées une fois pour toutes. $Q_p$ n'est pas qualifié de quantité économique : dans le protocole E5, c'est également une variable de décision à optimiser, pas une couverture fixe.
...
Prophet fournit une prévision mensuelle de référence entraînée sur 2015--2016 et appliquée à 2017 ; DataCo exécute la demande sous forme de commandes clientes journalières qui traversent le pipeline Source, Make, Deliver ; le Recalibrator combine cette prévision à la demande observée au moyen d'un facteur $\alpha_j=\sqrt{j/N}$ dont les propriétés de bornage, de monotonie et de concavité justifient la forme, et dont l'efficacité empirique est mesurée sur le RUN 1 par une réduction de l'erreur absolue totale d'environ 95,06\,\%. E4 formalise le protocole d'optimisation opérationnelle à exécuter sur le cas ZENER, selon une exploration factorielle, un front de Pareto et une sélection AHP ; E5 formalise, sur le même principe, un protocole pour la planification multi-produit fondée sur la prévision. L'exécution et la validation statistique de ces deux protocoles restent à mener, une fois la condition terminale atteinte et la graine aléatoire exposée.
