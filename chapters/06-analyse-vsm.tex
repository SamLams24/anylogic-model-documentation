...
Le modèle distingue ces deux périmètres temporels afin de conserver une interprétation cohérente des indicateurs. Un ordre autonome reste nécessaire à l'explication du service de la commande, mais il n'entre pas dans le dénombrement des commandes clientes. La relation RDF \texttt{contributesToCustomer}\allowbreak\texttt{OrderFulfillment}, accompagnée de sa relation inverse, rend l'imputation vérifiable dans l'ABox. Le même registre alimente le Dashboard Global et les métriques Responsiveness.

Les jetons utilisés uniquement pour l'animation sont exclus des stocks et du WIP physiques. Cette règle évite qu'une représentation graphique soit comptée comme une entité de production. L'exécution étudiée porte sur une commande et un ordre interne; elle ne permet pas de vérifier le partage quantitatif d'une même reconstitution entre plusieurs commandes.

\section{Résultats temporels de l'exécution validée}

La paire Excel et ABox de l'exécution validée fournit les six valeurs centrales de la \cref{tab:vsm-six-indicateurs}. Les valeurs temporelles exactes proviennent de l'ABox; l'affichage est arrondi à deux décimales pour préserver la lisibilité.

\begin{table}[H]
  \centering
  \caption{Indicateurs VSM de l'exécution validée.}
...
L'attente représente environ 72,1 pour cent du Lead Time de commande. Cette proportion décrit la structure temporelle de l'exécution, sans établir à elle seule la cause du retard client. Le délai fournisseur, le traitement de reconstitution, la dynamique Make et la livraison appartiennent à une même trajectoire perturbée; aucune exécution de référence synchronisée ne permet d'isoler leur contribution causale respective.
...
Les six indicateurs sont concordants entre Excel et RDF et peuvent être utilisés comme entrées du pipeline de performance. Leur validation reste limitée à cette exécution. Aucun effet net du retard n'est quantifié, faute d'un protocole comparatif contrôlé.
