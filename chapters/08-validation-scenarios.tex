\chapter{Validation expérimentale du scénario retenu}
\label{chap:validation-scenarios}

\section{Configuration expérimentale}

La validation expérimentale repose sur une exécution primaire complète du modèle consolidé. Elle vérifie le comportement combiné d'une rupture initiale de produit fini, d'une absence de GPL et d'un retard déterministe sur la première réception de GPL. L'objectif n'est pas de comparer des politiques, mais de contrôler la cohérence de la chaîne de reconstitution, des relations de traçabilité et des indicateurs temporels jusqu'au PI.

L'exécution utilise le scénario \texttt{SCENARIO DISTRIBUTION}. Une commande cliente de 20 unités est lancée avec un stock initial de produit fini nul. Le GPL est nul, les bouteilles vides valent 250 unités et les accessoires 300 unités. Le retard fournisseur est actif sur \texttt{GPL\_VRAC}, avec un facteur 2 appliqué à la première réception seulement. Le délai ciblé passe de 6 h nominales à 12 h effectives.

\noindent Le runId associé est:
\begin{center}
\texttt{RUN\_1773129600000\_1788264883846}.
\end{center}
...
\begin{table}[H]
  \centering
  \caption{Chronologie compacte de l'exécution de validation.}
  \label{tab:chronologie-execution-vsm}
...
La propagation prouve qu'un écart local de Source est reformulé jusqu'à Deliver. Elle ne prouve pas que la révision minimise un coût global ni que les six heures supplémentaires expliquent seules le statut tardif. Une exécution sans retard construite avec le même modèle, les mêmes conditions et une graine contrôlée serait nécessaire pour cette attribution.
...
L'exécution contient aussi la détection d'un goulot sur \Agent{sM1.3.1}, une décision locale \texttt{REBALANCE}, un score AHP de 0,730 et les accusés d'application. Ces faits valident cette boucle locale dans le scénario retenu. L'AHP ne contribue pas directement au PI et aucun effet de la décision sur le score global ne peut être déduit de cette observation unique.

\section{Portée de la validation}

Les artefacts valident le modèle consolidé pour une commande, une reconstitution et une perturbation déterministe. Ils établissent la cohérence croisée du manifeste, des relations ontologiques, des flux, des temps VSM, de Responsiveness et de l'agrégation du PI. La structure ISA-95 exportée contient 190 équipements typés, 71 micro-activités et 71 relations d'affectation, conformément à la garde du modèle.

L'exécution ne valide pas le fonctionnement nominal, Return, un scénario qualité avec défauts, une panne, plusieurs commandes partageant un ordre de reconstitution, une politique optimale ou une généralisation aux opérations réelles. Elle ne fournit ni comparaison causale avec une référence sans retard, ni réplications permettant d'estimer la variabilité. Ces absences délimitent le résultat expérimental.
