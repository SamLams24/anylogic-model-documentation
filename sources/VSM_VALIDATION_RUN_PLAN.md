...
Un seul run sera exécuté après correction du modèle. Il doit vérifier la continuité temporelle entre `CMD_1`, `REAPPRO_1`, Source, Make et Deliver, puis contrôler que VSM, Responsiveness et le PI utilisent les mêmes définitions dans Excel et dans l'ABox.
...
`Dashboard Global` doit exposer Process Time, Waiting Time, Lead Time et PCE dans une même unité. Il faut vérifier `Lead Time = Process Time + Waiting Time`. Le PCE doit être calculé à partir de la valeur ajoutée estimée, et non fixé par construction.
...
Les individus VSM à contrôler sont `vsm_cycle_global`, `vsm_wait_global` et `vsm_lead_global`. La correction devrait ajouter un individu explicite pour la valeur ajoutée ou le PCE global afin d'éviter une reconstruction implicite. Chaque indicateur doit porter l'unité et le contexte d'observation.
...
3. L'attente globale provient d'états ou de files horodatés et non de `Lead Time multiplié par (1 - PCE)`.
4. Process Time, Waiting Time et Lead Time partagent la même unité et vérifient leur identité additive.
5. Le PCE est recalculé à partir d'une valeur ajoutée explicitement qualifiée d'estimée lorsqu'un taux configuré est utilisé.
